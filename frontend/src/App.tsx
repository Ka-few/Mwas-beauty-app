    ...
    <nav className="w-64 min-h-screen bg-purple-900 p-4 text-purple-100 flex flex-col justify-between shadow-xl">
      <div>
        ...
        <ul className="flex flex-col gap-2">
          ...
          {user.role === 'admin' && isFeatureAllowed('CONSUMABLES') && (
            <li>
          ...
          {user.role === 'admin' && (
            <li>
              <Link to="/sync-settings" className="block p-3 rounded hover:bg-purple-800 hover:text-gold-400 transition-colors">
                Sync Settings
              </Link>
            </li>
          )}
      ...
      <main className={`flex-1 p-4 ${!user ? 'w-full' : ''}`}>
        <Routes>
          ...
          <Route path="/consumables" element={
            <ProtectedRoute allowedRoles={['admin']}>
              {isFeatureAllowed('CONSUMABLES') ? <Consumables /> : <Navigate to="/sales" replace />}
            </ProtectedRoute>
          } />
